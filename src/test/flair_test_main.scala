...
// Flair's own suite, run WITHOUT fume: since Soundness 0.65.0 a `Suite` has no `main` (the host —
// normally fume — drives it through `invoke`), so this is the plain-`java` entry point `make
// test-plain` and CI use, printing one line per completed test and exiting with the suite's
// status (0 = passed, 1 = failures, 2 = the suite threw). `fume run` remains the full experience.
@main
def runTests(): Unit =
  val tally = Tally()
