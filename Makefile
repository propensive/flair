...
# Releases are cut by tagging, not by make. Bump `flairVersion`, merge it, and then `git tag -s
...
# only to say so.
release:
	@echo "Releases are triggered by tags, not by make. Bump flairVersion, merge it, then:" >&2
	@echo "" >&2
	@echo "    git tag -s X.Y.Z && git push --tags" >&2
	@echo "" >&2
	@echo "See doc/releasing.md, and propensive/.github." >&2
	@exit 1
